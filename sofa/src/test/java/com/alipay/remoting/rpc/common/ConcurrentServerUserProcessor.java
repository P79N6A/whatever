...
public class ConcurrentServerUserProcessor extends SyncUserProcessor<RequestBody> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ConcurrentServerUserProcessor.class);
    ...
    public ConcurrentServerUserProcessor() {
        ...
        this.delayMs = 0;
        this.executor = new ThreadPoolExecutor(50, 100, 60, TimeUnit.SECONDS, new ArrayBlockingQueue<Runnable>(1000), new NamedThreadFactory("Request-process-pool"));
    }
    ...
    public ConcurrentServerUserProcessor(long delay, int core, int max, int keepaliveSeconds, int workQueue) {
        this(delay);
