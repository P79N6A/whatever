...
public class FailoverClusterTest extends ActivelyDestroyTest {
    ...
    public void testSingleServer() {
        ...
        ConsumerConfig<HelloService> consumerConfig2 = new ConsumerConfig<HelloService>().setInterfaceId(HelloService.class.getName()).setDirectUrl("bolt://127.0.0.1:22222").setTimeout(1000).setCluster("failover").setRetries(2) // 失败后自动重试2次
                .setRegister(false);
    ...
    public void testMultiServer() {
        ...
        ConsumerConfig<HelloService> consumerConfig = new ConsumerConfig<HelloService>().setInterfaceId(HelloService.class.getName()).setDirectUrl("bolt://127.0.0.1:22223;bolt://127.0.0.1:22224").setTimeout(1000).setCluster("failover").setRetries(1) // 失败后重试一次
                .setRegister(false);
        final HelloService helloService = consumerConfig.refer();
        int count2 = 0;
        for (int i = 0; i < 4; i++) {
