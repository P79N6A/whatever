...
public class LocalPreferenceLoadBalancerTest extends BaseLoadBalancerTest {
    ...
    public void doSelect() throws Exception {
        ...
        SofaRequest request = new SofaRequest();
        {
            ...
            for (int i = 0; i < localps; i++) {
                Assert.assertTrue(avg * 0.9 < cnt.get(22000 + i) && avg * 1.1 > cnt.get(22000 + i)); // 随机偏差不会太大，应该不超过10%
            }
        }
        {
            for (int i = 0; i < size; i++) {
            ...
            for (int i = 1; i < localps; i++) {
                Assert.assertTrue(per * i * 0.9 < cnt.get(22000 + i) && per * i * 1.1 > cnt.get(22000 + i)); // 随机偏差不会太大，应该不超过10%
            }
        }
    }
