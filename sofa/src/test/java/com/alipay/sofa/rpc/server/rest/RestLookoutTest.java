...
public class RestLookoutTest extends ActivelyDestroyTest {
    ...
    public static void beforeCurrentClass() {
        ...
        // 只有1个线程 执行
        serverConfig = new ServerConfig().setStopTimeout(10000).setPort(8802).setProtocol(RpcConstants.PROTOCOL_TYPE_REST).setContextPath("/xyz");
    ...
    public void beforeMethod() {
        ...
        providerConfig = new ProviderConfig<RestService>().setInterfaceId(RestService.class.getName()).setRef(new RestServiceImpl()).setServer(serverConfig).setBootstrap("rest")
                //.setParameter(RpcConstants.CONFIG_HIDDEN_KEY_WARNING, "false")
                .setRegister(false).setApplication(serverApplication);
