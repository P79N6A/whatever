...
public class HystrixFilterSyncTest extends ActivelyDestroyTest {
    ...
    private ProviderConfig<HelloService> defaultServer(int sleep) {
        ...
        ProviderConfig<HelloService> providerConfig = new ProviderConfig<HelloService>().setInterfaceId(HelloService.class.getName()).setRef(new HelloServiceImpl(sleep)).setServer(serverConfig);
        return providerConfig;
    }

    private ConsumerConfig<HelloService> defaultClient() {
        return new ConsumerConfig<HelloService>().setInterfaceId(HelloService.class.getName()).setDirectUrl("bolt://127.0.0.1:22222").setParameter(HystrixConstants.SOFA_HYSTRIX_ENABLED, String.valueOf(true));
    }
