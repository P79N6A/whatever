...
public class ClientTransportFactory {
        ...
        if (needDestroy) {
            if (disconnectTimeout > 0) { // 需要等待结束时间
                ...
                if (count > 0) { // 有正在调用的请求
                    ...
                    while (clientTransport.currentRequests() > 0 && RpcRuntimeContext.now() - start < disconnectTimeout) { // 等待返回结果
                        try {
                            Thread.sleep(10);
    ...
    public static ClientTransport getReverseClientTransport(String channelKey) {
        return REVERSE_CLIENT_TRANSPORT_MAP != null ? REVERSE_CLIENT_TRANSPORT_MAP.get(channelKey) : null;
    }
