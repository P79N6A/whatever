...
public class RpcInternalContext implements Cloneable {
    ...
    public RpcInternalContext setAttachment(String key, Object value) {
        ...
        if (!ATTACHMENT_ENABLE) {
            ...
            if (!isValidInternalParamKey(key)) { // 打开附件传递功能，只能传 "_" 和 "." 开头的Key
                throw new IllegalArgumentException("key must start with" + RpcConstants.INTERNAL_KEY_PREFIX + " or " + RpcConstants.HIDE_KEY_PREFIX);
            }
        }
        if (value == null) {
    ...
    public String toString() {
        return super.toString() + "{" + "future=" + future + ", localAddress=" + localAddress + ", remoteAddress=" + remoteAddress + ", attachments=" + attachments + ", stopWatch=" + stopWatch + ", providerSide=" + providerSide + ", providerInfo=" + providerInfo + '}';
    }
