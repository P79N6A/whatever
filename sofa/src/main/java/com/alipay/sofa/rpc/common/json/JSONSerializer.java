...
public class JSONSerializer {
    ...
    public static String serialize(Object object, boolean addType) {
        if (object == null) {
            ...
            return '\"' + tmp.replace("\"", "\\\"").replace("\b", "\\b").replace("\t", "\\t").replace("\r", "\\r").replace("\f", "\\f").replace("\n", "\\n") + '\"';
            ...
            while (itr.hasNext()) {
                ...
                sb.append(serialize(entry.getKey(), addType)).append(':').append(serialize(entry.getValue(), addType)).append(',');
            }
