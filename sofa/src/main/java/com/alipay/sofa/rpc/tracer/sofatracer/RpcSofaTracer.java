...
public class RpcSofaTracer extends Tracer {
    ...
    public RpcSofaTracer() {
        ...
        //构造 RPC 的 tracer 实例
        sofaTracer = new SofaTracer.Builder(RPC_TRACER_TYPE).withClientReporter(clientReporter).withServerReporter(serverReporter).build();
    }
    ...
    public void clientBeforeSend(SofaRequest request) {
        ...
        if ((providerInfo = rpcInternalContext.getProviderInfo()) != null && providerInfo.getRpcVersion() >= 50100) { // 版本>5.1.0
            //新调用新:缓存在 Request 中
