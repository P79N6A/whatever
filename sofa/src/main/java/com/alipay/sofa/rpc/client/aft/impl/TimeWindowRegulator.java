...
public class TimeWindowRegulator implements Regulator {
     ...
     * 度量线程池
     */
    private final ScheduledService measureScheduler = new ScheduledService("AFT-MEASURE", ScheduledService.MODE_FIXEDRATE, new MeasureRunnable(), 1, 1, TimeUnit.SECONDS);
     ...
     * 计算线程池
     */
    private final ExecutorService regulationExecutor = ThreadPoolUtils.newFixedThreadPool(2, new LinkedBlockingQueue<Runnable>(16), new NamedThreadFactory("AFT-REGULATION"));
    ...
    @Override
    public void init() {
        String measureStrategyAlias = RpcConfigs.getOrDefaultValue(RpcOptions.AFT_MEASURE_STRATEGY, "serviceHorizontal");
        String regulationStrategyAlias = RpcConfigs.getOrDefaultValue(RpcOptions.AFT_REGULATION_STRATEGY, "serviceHorizontal");
        ...
        measureStrategy = ExtensionLoaderFactory.getExtensionLoader(MeasureStrategy.class).getExtension(measureStrategyAlias);
        regulationStrategy = ExtensionLoaderFactory.getExtensionLoader(RegulationStrategy.class).getExtension(regulationStrategyAlias);
        degradeStrategy = ExtensionLoaderFactory.getExtensionLoader(DegradeStrategy.class).getExtension(degradeStrategyAlias);
        recoverStrategy = ExtensionLoaderFactory.getExtensionLoader(RecoverStrategy.class).getExtension(recoverStrategyAlias);
        ...
        public void run() {
            ...
            for (MeasureResultDetail measureResultDetail : measureResultDetails) {
                try {
                    ...
                    LOGGER.errorWithApp(measureResult.getMeasureModel().getAppName(), "Error when doRegulate: " + e.getMessage(), e);
                }
            }
        }
        ...
        void doRegulate(MeasureResultDetail measureResultDetail) {
            ...
            if (isDegradeEffective) {
                ...
                if (measureState.equals(MeasureState.ABNORMAL)) {
                    ...
                    if (!isReachMaxDegradeIpCount) {
                        ...
                        if (LOGGER.isInfoEnabled(appName)) {
                            LOGGER.infoWithApp(appName, LogCodes.getLog(LogCodes.INFO_REGULATION_ABNORMAL_NOT_DEGRADE, "Reach degrade number limit.", statDimension.getService(), statDimension.getIp(), statDimension.getAppName()));
                        }
                    }
                ...
                if (measureState.equals(MeasureState.ABNORMAL)) {
                    ...
                    String appName = measureResult.getMeasureModel().getAppName();
                    if (LOGGER.isInfoEnabled(appName)) {
                        LOGGER.infoWithApp(appName, LogCodes.getLog(LogCodes.INFO_REGULATION_ABNORMAL_NOT_DEGRADE, "Degrade switch is off", statDimension.getService(), statDimension.getIp(), statDimension.getAppName()));
                    }
                }
            }
        }
