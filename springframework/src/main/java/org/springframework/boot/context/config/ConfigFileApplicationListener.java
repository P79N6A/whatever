...
public class ConfigFileApplicationListener implements EnvironmentPostProcessor, SmartApplicationListener, Ordered {
    ...
    private void onApplicationEnvironmentPreparedEvent(ApplicationEnvironmentPreparedEvent event) {
        // 从META-INF/spring.factories加载key为org.springframework.boot.env.EnvironmentPostProcessor的类，并实例化
    ...
    private class Loader {
        ...
        public void load() {
            ...
            MapPropertySource defaultProperties = (MapPropertySource) this.environment.getPropertySources().get(DEFAULT_PROPERTIES);
        ...
        private void replaceDefaultPropertySourceIfNecessary(MapPropertySource defaultProperties) {
            if (defaultProperties != null) {
                this.environment.getPropertySources().replace(DEFAULT_PROPERTIES, new FilteredDefaultPropertySource(DEFAULT_PROPERTIES, defaultProperties.getSource()));
            }
        }
        ...
        private DocumentConsumer addToLoaded(BiConsumer<MutablePropertySources, PropertySource<?>> addMethod, boolean checkForExisting) {
            return (profile, document) -> {
                ...
                // 一个profile对应一个MutablePropertySources
        ...
        private void load(Profile profile, DocumentFilterFactory filterFactory, DocumentConsumer consumer) {
            // 遍历位置，Environment没配置就默认classpath:/,classpath:/config/,file:./,file:./config/
            getSearchLocations().forEach((location) -> {
                boolean isFolder = location.endsWith("/");
                // Environment没配置就默认application
        ...
        private void load(PropertySourceLoader loader, String location, Profile profile, DocumentFilter filter, DocumentConsumer consumer) {
            try {
                ...
                // PropertySourceLoader加载成PropertySource列表，再转Document列表
        ...
        private void resetEnvironment(MapPropertySource defaultProperties) {
            ...
            activeProfiles.addAll(this.processedProfiles.stream().filter((profile) -> profile != null && !profile.isDefaultProfile()).map(Profile::getName).collect(Collectors.toList()));
    ...
    private static class FilteredDefaultPropertySource extends MapPropertySource {
        ...
        private static final List<String> FILTERED_PROPERTY = Arrays.asList("spring.profiles.active", "spring.profiles.include");
        ...
        public String[] getPropertyNames() {
            return Arrays.stream(super.getPropertyNames()).filter((name) -> !isFilteredProperty(name)).toArray(String[]::new);
        }
