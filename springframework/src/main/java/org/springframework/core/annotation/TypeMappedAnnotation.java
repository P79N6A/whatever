...
final class TypeMappedAnnotation<A extends Annotation> extends AbstractMergedAnnotation<A> {
    ...
    public MergedAnnotation<?> getParent() {
        ...
        return new TypeMappedAnnotation<>(parentMapping, this.classLoader, this.source, this.rootAttributes, this.valueExtractor, this.aggregateIndex, this.resolvedRootMirrors);
    }
    ...
    private Object getValue(int attributeIndex, boolean useConventionMapping, boolean forMirrorResolution) {
        ...
        if (!forMirrorResolution) {
            attributeIndex = (mapping.getDepth() != 0 ? this.resolvedMirrors : this.resolvedRootMirrors)[attributeIndex];
        }
        if (attributeIndex == -1) {
            return null;
        }
    ...
    private static <A extends Annotation> TypeMappedAnnotation<A> createIfPossible(AnnotationTypeMapping mapping, @Nullable Object source, @Nullable Object rootAttribute, BiFunction<Method, Object, Object> valueExtractor, int aggregateIndex, IntrospectionFailureLogger logger) {
        try {
            ...
            if (logger.isEnabled()) {
                ...
                String item = (mapping.getDepth() == 0 ? "annotation " + type : "meta-annotation " + type + " from " + mapping.getRoot().getAnnotationType().getName());
