...
// readInt与getInt区别在于是否自动维护读索引，readInt将增加读索引，getInt则不会影响索引

// 空参数的方法类似getter，带参数的方法类似setter
...
// 上述后两个方法，可操作读写索引，还有以下方法可以操作读写索引：
...
// 可在已有缓冲区上创建派生缓冲区，这些视图维护各自独立的写索引、读索引以及标记索引，但他们和原生缓冲区共享内部字节数据
// 创建视图的方法有：duplicate()，slice()及slice(int, int)
// 使用这两个方法时，注意结合确定是否调用retain()增加引用计数
// 如果想拷贝缓冲区，期望维护特有的字节数据而不是共享字节数据，可使用copy()
