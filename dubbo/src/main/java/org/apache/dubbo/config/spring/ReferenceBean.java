...
/**
 * ReferenceFactoryBean，Dubbo服务引用的时机有两个
 * 第一个是在Spring容器调用ReferenceBean的afterPropertiesSet方法时引用服务
 * 第二个是在ReferenceBean对应的服务被注入到其他类中时引用
 * 区别在于，第一个是饿汉式的，第二个是懒汉式的，默认懒汉式
 * 如果需要使用饿汉式，可通过配置 <dubbo:reference> 的init属性开启
 */
