...
// 可以在 for 循环中做删除操作 删除之后也都是拷贝重写赋值
// ArrayList 用 for 循环删除实际用迭代器的 next 方法
//     会比较 expectedModCount 和 modCount 是否相等，
...
//    缺点就是可能读取的不是最新的值
    ...
    // 原因在于：在创建一个迭代器时，它会拷贝一份列表数据，这样即使操作列表也不会影响迭代器，缺点和前面一样，可能无法反映数据的最新状态。
    // Git 多人协作流程：
