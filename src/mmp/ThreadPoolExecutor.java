...
public class ThreadPoolExecutor extends AbstractExecutorService {
    ...
    // int一共有32位，runState至少需要3位，故workCount只能有29位，所以线程池的有效线程数最多为...
    ...
    private static final int TIDYING = 2 << COUNT_BITS; // 所有的任务已终止，ctl记录的"任务数量"为0，线程池会变为TIDYING状态
    private static final int TERMINATED = 3 << COUNT_BITS; // 线程池彻底终止 TIDYING状态时，执行完terminated()之后
    ...
    private boolean addWorker(Runnable firstTask, boolean core) {
        retry:
        /*首先会再次检查线程池是否处于运行状态，核心线程池中是否还有空闲线程，
 都满足条件过后则会调用compareAndIncrementWorkerCount先将正在运行的线程数+1，
 数量自增成功则跳出循环，自增失败则继续从头继续循环*/
        for (; ; ) {
            int c = ctl.get();
            int rs = runStateOf(c);
            ...
            for (; ; ) {
                ...
                // 如果是 true，则创建 core 核心线程，如果是 false ，则创建 maximumPoolSize 线程
        ...
        try {
            while (task != null || (task = getTask()) != null) {
                ...
                try {
                    ...
                    try {
                        // 首先执行 firstTask 的 run 方法，然后循环获取阻塞队列中的任务，并调用他们的 run 方法
                        task.run();
    ...
    public void execute(Runnable command) {
        ...
        // 2 核心池已满，但任务队列未满，如果线程池正在运行，并且添加进队列成功
    ...
    // 都是通过遍历线程池中的线程，逐个调用 Thread.interrup() 来中断线程，所以一些无法响应中断的任务可能永远无法停止（比如 Runnable）
    ...
    // 子类重写 beforeExecute, afterExecute, terminated 方法
